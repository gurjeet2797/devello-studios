...
// Supports: construction, custom_product, software_build, consultation
async function sendTailoredFormEmail({
  formType, // 'construction', 'custom_product', 'software_build', 'consultation'
  ...
  try {
    ...
    const formConfig = {
      construction: {
        title: 'New Construction Service Request',
        greeting: 'A new construction service request has been submitted.',
        icon: '🏗️'
      },
      custom_product: {
    ...
    if (formType === 'construction' || formType === 'custom_product') {
      mainContent += `
        <div style="margin-top: 24px; padding: 20px; background: #f9fafb; border-radius: 8px; border: 1px solid #e5e7eb;">
