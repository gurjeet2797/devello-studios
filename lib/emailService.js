...
module.exports = {
  ...
  sendAdminBuildRequestNotification,
  verifyConnection,
  getTransporter,
...
// Send admin notification for build request
async function sendAdminBuildRequestNotification({
  productName,
  productTagline,
  productDescription,
  customerName,
  ...
  try {
    console.log('[EMAIL_SERVICE] [ADMIN_BUILD_REQUEST] Starting admin build request notification', {
      productName,
      customerEmail,
      timestamp: new Date().toISOString()
    });
    ...
    const mailTransporter = getTransporter();
    if (!mailTransporter) {
      console.error('[EMAIL_SERVICE] [ADMIN_BUILD_REQUEST] [ERROR] SMTP not configured');
      return { success: false, error: 'SMTP not configured' };
    }
    ...
    const content = `
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
      <p style="margin-top: 0; font-size: 16px;">A new app build request has been submitted and requires your attention.</p>
      
      <div style="background: ${serviceColor.lighterBg}; border-radius: 12px; padding: 20px; margin: 20px 0;">
        <h3 style="margin-top: 0; color: ${serviceColor.primary}; font-size: 18px;">Product Details</h3>
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 8px 0; font-weight: 600; color: #333;">Product Name:</td>
            <td style="padding: 8px 0; color: #666;">${productName}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; font-weight: 600; color: #333;">Tagline:</td>
            <td style="padding: 8px 0; color: #666;">${productTagline || 'N/A'}</td>
          </tr>
          <tr>
            <td style="padding: 8px 0; font-weight: 600; color: #333; vertical-align: top;">Description:</td>
            <td style="padding: 8px 0; color: #666;">${productDescription || 'N/A'}</td>
          </tr>
        </table>
      </div>

      <div style="background: ${serviceColor.lighterBg}; border-radius: 12px; padding: 20px; margin: 20px 0;">
        <h3 style="margin-top: 0; color: ${serviceColor.primary}; font-size: 18px;">Contact Information</h3>
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
          ...
          <tr>
            <td style="padding: 8px 0; font-weight: 600; color: #333;">Email:</td>
            <td style="padding: 8px 0; color: #666;"><a href="mailto:${customerEmail}" style="color: ${serviceColor.primary};">${customerEmail}</a></td>
          </tr>
          <tr>
            <td style="padding: 8px 0; font-weight: 600; color: #333;">Phone:</td>
            <td style="padding: 8px 0; color: #666;"><a href="tel:${customerPhone}" style="color: ${serviceColor.primary};">${customerPhone}</a></td>
          </tr>
          <tr>
            <td style="padding: 8px 0; font-weight: 600; color: #333;">Idea ID:</td>
            <td style="padding: 8px 0; color: #666;">${ideaId}</td>
          </tr>
        </table>
      </div>
      ${showcaseImage ? `
      <div style="background: ${serviceColor.lighterBg}; border-radius: 12px; padding: 20px; margin: 20px 0; text-align: center;">
        <h3 style="margin-top: 0; color: ${serviceColor.primary}; font-size: 18px;">Product Showcase</h3>
        <img src="${showcaseImage}" alt="${productName} showcase" style="max-width: 100%; border-radius: 8px; margin-top: 10px;" />
      </div>
      ` : ''}
    </div>
    `;
    ...
    const mailOptions = {
      from: `"${fromName}" <${fromEmail}>`,
      to: adminEmail,
      subject: `New Build Request: ${productName}`,
      html: html,
      replyTo: customerEmail || fromEmail,
      headers: {
        'X-Priority': '1',
    ...
    console.log('[EMAIL_SERVICE] [ADMIN_BUILD_REQUEST] Sending admin build request notification via SMTP', {
      from: `${fromName} <${fromEmail}>`,
      to: adminEmail,
      productName
    });
    ...
    const info = await mailTransporter.sendMail(mailOptions);
    console.log('[EMAIL_SERVICE] [ADMIN_BUILD_REQUEST] Admin build request notification sent successfully', {
      messageId: info.messageId,
      response: info.response
    });
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error('[EMAIL_SERVICE] [ADMIN_BUILD_REQUEST] [ERROR] Error sending admin build request notification:', {
      error: error.message,
      stack: error.stack,
      code: error.code
    });
...
module.exports.sendAdminBuildRequestNotification = sendAdminBuildRequestNotification;
